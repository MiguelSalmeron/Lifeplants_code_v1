...
export function renderPlantGrid(plantKeys, allPlants, favorites, containerId = 'plantGridContainer') {
    ...
    plantKeys.forEach(key => {
        ...
        const srcset = generateSrcset(plant.imageCard);
    ...
    section.innerHTML = `
        ...
        <div class="plant-detail-container">
            ...
            <div class="plant-detail-content">
                <div class="section collapsible open"><h2 class="section-title">¿Para qué sirve?</h2><div class="section-content"><p>${plant.whatIsItFor}</p></div></div>
...
export function renderPlantOfTheWeek(plant, plantKey) {
    ...
    document.getElementById('potwImage').src = plant.imageCard;
    document.getElementById('potwImage').srcset = srcset;
    document.getElementById('potwImage').sizes = '(max-width: 768px) 100vw, 400px';

    document.getElementById('potwName').textContent = plant.name;
    document.getElementById('potwDescription').textContent = plant.whatIs;
    document.getElementById('potwButton').dataset.plantKey = plantKey;
...
export function populateFilterOptions(allPlants) {
    ...
    Object.values(allPlants).forEach(plant => {
        if (plant.uso) plant.uso.forEach(u => filters.uso.add(u.toLowerCase()));
        if (plant.region) plant.region.forEach(r => filters.region.add(r.toLowerCase()));
        if (plant.tipo) filters.tipo.add(plant.tipo.toLowerCase());
    });
